...
const statOfDirectory = async (directoryPath: string, depth: number, log: boolean) => {
  if (log && depth < 3) {
  ...
  const commandLineArgs = require("command-line-args");
  const os = require("os");
  const { readdir, stat, opendir } = require("fs/promises");
  ...
  const statOfDirectory = async (directoryPath, depth, log) => {
    try {
      const fd = await opendir(directoryPath);
      fd.close();
